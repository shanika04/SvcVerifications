...
public class GwtPackageServiceImpl extends OsgiRemoteServiceServlet implements GwtPackageService {
  ...
  public List<GwtDeploymentPackage> findDeviceDeploymentPackages(GwtXSRFToken xsrfToken)
    ...
    DeploymentAdmin deploymentAdmin =
        ServiceLocator.getInstance().getService(DeploymentAdmin.class);
    ...
    if (deploymentPackages != null) {
      for (DeploymentPackage deploymentPackage : deploymentPackages) {
        ...
        gwtDeploymentPackage.setVersion(
            GwtSafeHtmlUtils.htmlEscape(deploymentPackage.getVersion().toString()));
        ...
        if (bundleInfos != null) {
          for (BundleInfo bundleInfo : bundleInfos) {
            ...
            gwtBundleInfo.setVersion(
                GwtSafeHtmlUtils.htmlEscape(bundleInfo.getVersion().toString()));
  ...
  public void uninstallDeploymentPackage(GwtXSRFToken xsrfToken, String packageName)
    ...
    DeploymentAgentService deploymentAgentService =
        ServiceLocator.getInstance().getService(DeploymentAgentService.class);
    try {
      deploymentAgentService.uninstallDeploymentPackageAsync(
          GwtSafeHtmlUtils.htmlEscape(packageName));
    } catch (Exception e) {
  ...
  @Override
  @PostMapping
  public GwtMarketplacePackageDescriptor getMarketplacePackageDescriptor(
      GwtXSRFToken xsrfToken, String nodeId) throws GwtKuraException {
    checkXSRFToken(xsrfToken);
    ...
    try {
      ...
      */
      ...
      //            logger.warn("failed to get deployment package descriptior from Eclipse
      // Marketplace", e);
      throw new GwtKuraException(GwtKuraErrorCode.INTERNAL_ERROR, e);
    } finally {
      if (connection != null) {
  ...
  */
  ...
  @Override
  public void installPackageFromMarketplace(
      GwtXSRFToken xsrfToken, GwtMarketplacePackageDescriptor descriptor) throws GwtKuraException {
    //        checkXSRFToken(xsrfToken);
    //        try {
    //            ServiceLocator.applyToServiceOptionally(DeploymentAgentService.class,
    ...
    //                logger.info("Installing deployment package from Eclipse Marketplace, URL
    // {}...", dpUrl);
  ...
  private static class MarketplaceFeedbackEventHandler implements EventHandler {
    ...
    private MarketplaceFeedbackEventHandler(GwtMarketplacePackageDescriptor descriptor) {
      ...
      this.registration =
          FrameworkUtil.getBundle(GwtPackageServiceImpl.class)
              .getBundleContext()
    ...
    private void sendSuccessFeedbackToMarketplace() {
      ...
      try {
        //                logger.info("Sending successful install feedback to Eclipse
        // Marketplace...");
        ...
        //                logger.warn("Sending successful install feedback to Eclipse
        // Marketplace...failure", e);
      } finally {
        if (connection != null) {
          connection.disconnect();
        }
      ...
      if (eventUrl != null && eventUrl.equals(this.descriptor.getDpUrl())) {
        try {
          ...
          if (successful != null && successful) {
            //                        logger.info("Installing deployment package from Eclipse
            // Marketplace, URL {}...success", dpUrl);
            sendSuccessFeedbackToMarketplace();
          } else {
            //                        logger.info("Installing deployment package from Eclipse
            // Marketplace, URL {}...failure", dpUrl);
          }
        } finally {
